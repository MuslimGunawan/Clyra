        ...
        <div className="w-full max-w-md bg-[#0c0e18] border border-indigo-500/30 rounded-3xl p-6 sm:p-8 shadow-2xl shadow-indigo-950/40 space-y-6 relative overflow-hidden">
          ...
          <form onSubmit={handleActivate} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-300 flex items-center justify-between">
                <span>{t("member.email_label")}</span>
                {queryEmail && (
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-300 flex items-center justify-between">
                <span>{t("member.password_label")}</span>
                <span className="text-[10px] text-slate-500 font-mono">Min. 6</span>
              </label>
              <div className="relative">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-300 block">
                {t("member.confirm_password_label")}
              </label>
              <div className="relative">
                <Lock className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500" />
                <input
            ...
            >
              {isLoading ? (
                ...
                <>
                  <span>🚀 {t("member.activate_btn")}</span>
          ...
          <div className="pt-2 border-t border-slate-800/80 text-center">
            <p className="text-[11px] text-slate-500 flex items-center justify-center gap-1 font-mono">
              ...
              <span>Multi-Device • Secure Cloud Sync</span>
            </p>
          </div>
        </div>
      </main>
...
export default function MemberActivatePage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#07090e] flex items-center justify-center text-slate-400 font-mono text-xs">Loading...</div>}>
      <ActivateContent />
    </Suspense>
  );
}
